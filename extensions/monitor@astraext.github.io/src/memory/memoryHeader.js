...
export default GObject.registerClass(class MemoryHeader extends Header {
    constructor() {
        super('Memory Header');
        this.buildIcon();
        this.buildGraph();
        ...
        Utils.memoryMonitor.listen(this.tooltipMenu, 'memoryUsage', () => {
            ...
            if (Config.get_boolean('memory-header-tooltip-value')) {
                ...
                if (!usage || !usage.used || isNaN(usage.used))
                    ...
                    values.push(`${Utils.formatBytes(usage.used, unit, figures)}`);
            }
            if (Config.get_boolean('memory-header-tooltip-free')) {
                ...
                if (!usage || !usage.free || isNaN(usage.free))
                    ...
                    values.push(`${Utils.formatBytes(usage.free, unit, figures)}`);
            }
            if (values.length === 0)
                values.push('-');
