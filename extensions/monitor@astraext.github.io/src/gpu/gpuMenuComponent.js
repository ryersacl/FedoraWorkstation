...
export default class GpuMenuComponent {
    ...
    createInfoPopup(sourceActor, gpuInfo) {
        ...
        if (domain.length === 2) {
            ...
            pciLabel.clutterText.set_markup(`<span alpha="60%">${domain[0]}</span> : <b>${domain[1]}</b> : ${gpuInfo.bus} : <span font_scale="small-caps">${gpuInfo.slot}</span>`);
        ...
        let otherInfoHash = null;
        const splitCharacters = [' ', '-', '(', ')', '[', ']', '{', '}', ';'];
        popup.updateData = (data) => {
            if (otherInfoHash === data.info.pipes)
                return;
            if (Utils.deepEqual(otherInfoHash, data.info.pipes))
                return;
    ...
    createActivityPopup(sourceActor, _gpuInfo) {
        const popup = new MenuBase(sourceActor, 0.05);
    ...
    createVramPopup(sourceActor, _gpuInfo) {
        const popup = new MenuBase(sourceActor, 0.05);
    ...
    createTopProcessesPopup(sourceActor, _gpuInfo) {
        const popup = new MenuBase(sourceActor, 0.05);
    ...
    createSensorsPopup(sourceActor, _gpuInfo) {
        const popup = new MenuBase(sourceActor, 0.05);
