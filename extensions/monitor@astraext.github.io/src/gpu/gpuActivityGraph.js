...
export default GObject.registerClass(class GpuActivityGraph extends GraphBase {
    constructor(params) {
        super(params);
        Config.connect(this, 'changed::gpu-header-activity-graph-color1', this.setStyle.bind(this));
    }
    setStyle() {
        const lightTheme = Utils.themeStyle === 'light';
        this.colors = [
            Utils.parseRGBA(Config.get_string('gpu-header-activity-graph-color1'), 'rgba(29,172,214,1.0)'),
        ];
