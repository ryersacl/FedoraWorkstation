...
export default GObject.registerClass(class GpuMemoryGraph extends GraphBase {
    constructor(params) {
        super(params);
        Config.connect(this, 'changed::gpu-header-memory-graph-color1', this.setStyle.bind(this));
    }
    setStyle() {
        const lightTheme = Utils.themeStyle === 'light';
        this.colors = [
            Utils.parseRGBA(Config.get_string('gpu-header-memory-graph-color1'), 'rgba(29,172,214,1.0)'),
        ];
