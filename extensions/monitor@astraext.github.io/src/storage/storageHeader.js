...
export default GObject.registerClass(class StorageHeader extends Header {
    constructor() {
    ...
    buildPercentage() {
        ...
        Utils.storageMonitor.listen(this.percentage, 'storageUsage', this.updatePercentage.bind(this));
    }
    updatePercentage() {
        if (!this.visible)
            return;
    ...
    buildSpeed() {
        ...
        Utils.storageMonitor.listen(this.speedContainer, 'storageIO', this.updateSpeed.bind(this));
    }
    updateSpeed() {
        if (!this.visible)
            return;
        ...
        if (usage) {
            ...
            read = Utils.formatBytesPerSec(bytesReadPerSec, unit, maxFigures);
            write = Utils.formatBytesPerSec(bytesWrittenPerSec, unit, maxFigures);
        }
        ...
        const updateTooltip = () => {
            ...
            if (Config.get_boolean('storage-header-tooltip-io')) {
                ...
                if (usage) {
                    ...
                    maxFigures = Math.max(1, Math.min(4, maxFigures));
                    values.push('↑' +
                        Utils.formatBytesPerSec(usage.bytesReadPerSec, unit, maxFigures));
                    values.push('↓' +
                        Utils.formatBytesPerSec(usage.bytesWrittenPerSec, unit, maxFigures));
                }
            }
            if (values.length === 0)
                values.push('-');
